...
def forward_process(batch, prompt_index, mask_id, b, l, target_len, k, shuffled_seq):
    ...
    target_is_mask = indices < x.unsqueeze(1)
    for i in range(b):
        # is_mask[i] = is_mask[i][torch.randperm(target_len)]
        target_is_mask[i] = target_is_mask[i][shuffled_seq[i]]

    is_mask = torch.zeros(b, l, dtype=torch.bool, device=batch.device)
    is_mask[:, ~prompt_index] = target_is_mask
...
@torch.no_grad()
def get_joint_log_likelihood(model, seq, prompt_index, shuffled_seq, k, mc_num=128, batch_size=16, cfg_scale=0., mask_id=126336):
    '''
    Args:
