...
def forward_process(batch, prompt_index, mask_id, b, l, target_len, k, shuffled_seq):
    # b, l = batch.shape
    ...
    for i in range(b):
        ...
        is_mask[i] = is_mask[i][shuffled_seq[i]]

    is_mask = torch.cat((is_mask, torch.zeros(b, prompt_index.sum(), dtype=torch.bool, device=batch.device)), dim=1)
...
@torch.no_grad()
def get_reverse_log_likelihood(model, seq, prompt_index, shuffled_seq, k, mc_num=128, batch_size=16, cfg_scale=0., mask_id=126336):
    '''
    Args:
    ...
    '''
    # seq = torch.concatenate([answer, prompt])[None, :]
    # seq = seq.repeat((batch_size, 1)).to(model.device)
    # prompt_index = torch.arange(seq.shape[1], device=model.device) >= len(answer)
    assert len(k) == len(shuffled_seq)
    b, l = seq.shape
    target_len = (l - prompt_index.sum()).item()
    loss_ = []
    for i in range(mc_num // batch_size):
        perturbed_seq, p_mask = forward_process(seq, prompt_index, mask_id, b, l, target_len, k[i], shuffled_seq[i])
